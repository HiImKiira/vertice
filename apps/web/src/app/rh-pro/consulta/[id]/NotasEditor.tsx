...
export function NotasEditor({ empleadoId, initial, ultimaActualizacion }: Props) {
  ...
  function guardar() {
    setMsg(null);
    start(async () => {
      const r = await guardarNotaAction(empleadoId, valor);
      if (r.ok) {
  ...
  return (
    <div>
      <textarea
        ...
        rows={5}
        placeholder="Observaciones internas — solo RH las ve. Ej: méritos, llamadas de atención, situación familiar relevante, etc."
